...
test('station-specific thresholds remain distinct',()=>{
  ...
  assert.equal(STATIONS.iwabuchi.chartMax,8);assert.equal(STATIONS.chisuibashi.chartMax,15);assert.equal(STATIONS.kumagaya.chartMax,8)
});
test('water summary returns selected station metadata and exact deltas',()=>{
  ...
  assert.equal(d.station.key,'kumagaya');assert.equal(d.latest.value,2.3);assert.equal(d.delta10,.1);assert.equal(d.delta60,.3);assert.equal(d.chartMax,8)
});
test('v0.8 frontend uses requested JMA, river, rail and subway links and no shelter or utility status containers',()=>{
  ...
  assert.match(html,/arajo\/index\.html/);assert.match(html,/river\.go\.jp\/index\/twninfo/);
  assert.match(html,/keisei\.co\.jp\/traininfo\/index\.php/);assert.match(html,/kotsu\.metro\.tokyo\.jp\/subway\//);
  assert.match(app,/chisuibashi/);assert.match(app,/kumagaya/);assert.match(app,/\/api\/radar-tile/);
