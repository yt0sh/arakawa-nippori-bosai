...
export async function cameraHandler(req,res){
 ...
 try{
  ...
  if(!/image\/jpeg/i.test(type)||bytes.length<1000||bytes.length>3000000||bytes[0]!==255||bytes[1]!==216||bytes[2]!==255)throw Error('ライブカメラJPEGを確認できません');
  res.statusCode=200;res.setHeader('Content-Type','image/jpeg');res.setHeader('Cache-Control','public, s-maxage=60, max-age=0, must-revalidate');res.setHeader('X-Content-Type-Options','nosniff');res.end(Buffer.from(bytes));
