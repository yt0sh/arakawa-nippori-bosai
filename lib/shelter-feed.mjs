...
// The same JSON feed used by the ward's hinanjyo-ichiran.js.
...
export function parseShelterFeed(input){
  ...
  const shelters=SHELTERS.map(target=>{
    ...
    const matches=rows.filter(a=>a.code===code ||
      (norm(a.name)===norm(target.name)&&norm(a.address)===norm(target.address)));
    const exact=matches.filter(a=>norm(a.name)===norm(target.name)&&norm(a.address)===norm(target.address));
    // A changed name/address, duplicate record or missing record must not be silently accepted.
    const a=matches.length===1&&exact.length===1?exact[0]:null;
    const raw=typeof a?.status==='string'?a.status.trim():'';
    return {...target,code,state:statusState(raw),status:raw||'開設状況を確認できません',
      recordUpdatedAt:a?.update_at||null,source:SHELTER_FEED};
  });
  return {state:'ok',shelters,source:SHELTER_FEED,publicSource:SOURCES.shelters,
    dataUpdatedAt:doc.Update_At||null,
    coverage:{matched:shelters.filter(s=>s.state!=='unknown').length,total:shelters.length}};
}
export async function shelterHandler(req,res){
  if(req.method!=='GET')return response(res,405,{error:'Method not allowed'});
  try{
    const data=parseShelterFeed(await fetchText(SHELTER_FEED));
    return response(res,200,{...data,retrievedAt:new Date().toISOString()});
