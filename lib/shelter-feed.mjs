...
export const SHELTER_CODES=Object.freeze({'ひぐらし小学校':'hn100240','諏訪台中学校':'hn100340','諏訪台ひろば館':'hn200100','西日暮里ふれあい館':'hn200160','第六日暮里小学校':'hn100230'});
export function statusState(value){const s=norm(value);if(/閉鎖|未開設|開設していません/.test(s))return'closed';if(/満員|受入停止/.test(s))return'full';if(/混雑/.test(s))return'crowded';if(/開設準備|準備中/.test(s))return'preparing';if(/開設中|開設済|開設しています|受入中|受け入れ中/.test(s)||s==='開設')return'open';return'unknown'}
export function parseShelterFeed(input){
 const doc=typeof input==='string'?JSON.parse(input):input;
 if(!Array.isArray(doc?.Hinanzyo?.features)||!doc.Hinanzyo.features.length)throw Error('避難所JSONの施設一覧を確認できません');
...
export async function shelterHandler(req,res){if(req.method!=='GET')return response(res,405,{error:'Method not allowed'});try{return response(res,200,{...parseShelterFeed(await fetchText(SHELTER_FEED)),retrievedAt:new Date().toISOString()})}catch(e){return response(res,502,{state:'error',error:String(e.message||e),source:SHELTER_FEED,retrievedAt:new Date().toISOString()})}}
